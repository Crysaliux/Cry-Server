...
const GroupModalView: React.FC = () => {
    ...
    const GroupModalSubmitReference = useRef<HTMLButtonElement>(null);
    ...
    const CreateGroup = async () => {
        if (!context_data.group_to_create && !submit_status) {
            SetSubmitStatus(true);
            if (GroupModalNameReference.current && GroupModalDescReference.current && GroupModalSubmitReference.current) {
                if (GroupModalNameReference.current.value !== '' && GroupModalDescReference.current.value !== '') {
                    const current_color = window.getComputedStyle(GroupModalSubmitReference.current).backgroundColor;
                    GroupModalSubmitReference.current.style.backgroundColor = `${current_color}`;
                ...
                context_data.SetError("Data load error. Can't process modal data");
            }
        }
    };
        ...
        <>
            {
                ...
                <>
                    <div className="modal border" id="group-modal">
                        ...
                        <div className="modal_choice" id="group-modal-choice">
                            <button className="button blue small nocopy" ref={GroupModalSubmitReference} onClick={CreateGroup}>Create Group</button>
